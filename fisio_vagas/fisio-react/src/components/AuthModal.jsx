...
export default function AuthModal({ isOpen, onClose, onAuthenticated }) {
  ...
  const handleLoginSubmit = async () => {
    ...
    try {
      ...
      setLoginError(
        msg.includes("400")
          ? "Código de 6 dígitos inválido ou expirado."
          : msg || "Erro na verificação do código.",
      );
    } finally {
      setLoginLoading(false);
    }
  ...
  const handleRegRegister = async () => {
    ...
    setRegLoading(true);
    try {
      // Cria a conta enviando apenas números conforme pedido "sem alterar o envio para api"
      const docClean = regDoc.replace(/\D/g, "");
      await registerOfertante({
        nome: regNome,
        documento: docClean,
        email: regEmail,
        tipo: regTipo,
      });
      // API separou o envio de e-mail na sua nova versão, temos que chamar no fluxo:
      await requestCode(regEmail);
      setRegCodeSent(true);
    } catch (err) {
      const msg = err.message || "";
      // Um erro 400 no cadastro geralmente indica duplicidade no backend
      setRegError(
        msg.includes("400")
          ? "Esse E-mail ou Documento já está cadastrado no sistema."
            ...
            <div className="auth-form active">
              ...
              {loginCodeSent ? (
                <>
                  ...
                  >
                    Um código foi enviado para <strong>{loginEmail}</strong>.
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLoginCodeInput(e.target.value.replace(/\D/g, ""))
                      }
                      autoFocus
                    />
                  </div>
            ...
            <div className="auth-form active">
              ...
              {!regCodeSent ? (
                <>
                  ...
                  <div className="form-group">
                    ...
                    <select
                      value={regTipo}
                      onChange={(e) => {
                        setRegTipo(e.target.value);
                        setRegDoc(""); // Limpa o doc ao trocar de tipo para evitar máscaras misturadas
                      }}
                      style={selectStyle}
                    >
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      value={regDoc}
                      onChange={(e) =>
                        setRegDoc(formatDoc(e.target.value, regTipo))
                      }
                    />
                  </div>
                  <div className="form-group">
                ...
                <>
                  ...
                  >
                    Conta criada! Um código de confirmação foi enviado para{" "}
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      value={regCodeInput}
                      onChange={(e) =>
                        setRegCodeInput(e.target.value.replace(/\D/g, ""))
                      }
                      autoFocus
                    />
                  </div>
