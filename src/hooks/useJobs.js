...
export function useJobs() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      // API retorna { dados: [...], paginacao: {...} } ou array direto
      setJobs(Array.isArray(data) ? data : (data.dados ?? data.posts ?? []));
    } catch (err) {
...
export function useMyJobs() {
  ...
  const fetchMyJobs = useCallback(async () => {
    ...
    try {
      ...
      setMyJobs(Array.isArray(data) ? data : (data.dados ?? data.posts ?? []));
    } catch (err) {
